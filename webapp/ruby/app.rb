  ...
  class App < Sinatra::Base
    ...
    helpers do
      ...
      def make_posts(where: nil, order: nil, param: nil, all_comments: false)
        ...
        results.to_a.map do |post|
          ...
          query = 'SELECT `comments`.*, `users`.`account_name` as user_name FROM `comments` JOINS `users` on `users`.`id` = `comments`.`user_id` WHERE `post_id` = ? ORDER BY `comments`.`created_at` DESC'
          unless all_comments
            query += ' LIMIT 3'
          end
    ...
    get '/image/:id.:ext' do
      ...
      if (params[:ext] == "jpg" && post[:mime] == "image/jpeg") ||
        ...
        File.open("../public/image/#{params[:id]}.#{params[:ext]}", "wb") do |f|
